...
# since ? and ! and . are most often associated with the end of a sentence, this is what the program will count.
    ...
    def punctuation():
        ...
        punc_str = "The distance between the two texts based on punctuation is {0}".format(dist)
        # Need to return vars inside function and then call function using return func() to retrieve values
        return profile1, profile2, punc_str
    ...
    # So all we have to do is call inner_conj() within conj and this allows the program the flexibility to use inner_conj within composite()
        ...
        conj_dist = round(mt.sqrt((count11-count22)**2),2)
        conj_str = "The distance between the two texts using conjunctions as a metric is {0}".format(conj_dist)
        return profile11, profile22, conj_str
    ...
    def unigrams():
        ...
        uni_dist_str = "The distance between the two texts using unigrams as a metric is {0}".format(uni_dist)
        return profile1, profile2, uni_dist_str
...
# function to count paragraphs
    ...
    def composite():
        ...
        sq = (count_values_1-count_values_2)**2
        ...
        distance = round(mt.sqrt(sq),2)
        distance_str = "The distance between the two texts is {0}".format(distance)
        #return results
        return merge1, merge2, distance_str


    # calls function will return features based on conditions met
